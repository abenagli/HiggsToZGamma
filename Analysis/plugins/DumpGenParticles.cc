#include "HiggsToZGamma/Analysis/plugins/DumpGenParticles.h"
  ...
  verbosity_(pSet.getParameter<bool>("verbosity"))
{
  ...
  outTree_ = GenTree(pSet.getUntrackedParameter<std::string>("treeName").c_str(),"Gen tree for H > Z+gamma studies");
  entry_ = 0;
}
...
void DumpGenParticles::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(size_t i = 0; i < genParticles.size(); ++i)
  {
           ...
           genParticle.status() == 23 ) )
    {
      ...
      // std::cout << "adding resonance " << resonance << std::endl;
